...
chmod 755 "$A"/usr/local/bin/{install-ifos,ifos-software,ifos-welcome,ifos-post-install,ifos-theme,ifos-launcher,ifos-bigpicture,ifos-update,ifos-update-terminal,ifos-term,ifos-lock,ifos-gpu,ifos-scale,install-yay,choose-mirror,Installation_guide,livecd-sound} 2>/dev/null || true
