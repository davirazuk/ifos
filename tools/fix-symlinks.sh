...
chmod 755 "$A"/usr/local/bin/{install-ifos,ifos-software,ifos-welcome,ifos-post-install,ifos-theme,ifos-launcher,ifos-bigpicture,ifos-update,install-yay,choose-mirror,Installation_guide,livecd-sound} 2>/dev/null || true
chmod 755 "$A"/usr/share/ifos/{post-install.sh,branding-sync.sh} 2>/dev/null || true
chmod 755 "$A"/etc/skel/.config/picom/launch.sh "$A"/etc/skel/.config/polybar/launch.sh "$A"/etc/skel/.config/polybar/scripts/*.sh \
