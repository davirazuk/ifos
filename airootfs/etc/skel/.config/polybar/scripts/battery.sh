...
# The same hexes as the bar's own [colors] block, so ifos-theme repaints these
...
case "$status" in
    ...
    *)
        ...
        if   [ "$cap" -le 10 ]; then colour="#f38ba8"
        elif [ "$cap" -le 25 ]; then colour="#f9e2af"
        fi
        ;;
esac
