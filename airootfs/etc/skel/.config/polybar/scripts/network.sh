...
#  name of the network, for everybody who installed IFOS rather than running it
#  from the USB stick.
#
...
if command -v nmcli >/dev/null 2>&1; then
    ...
    if (( nm_answered )); then
        printf '%%{F#f38ba8}󰖪%%{F#7fa392} offline\n'
        exit 0
    fi
fi
