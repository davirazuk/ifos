...
function fish_greeting --description 'IFOS greeting'
    set -l green (set_color 00a86b)
    set -l light (set_color 7ed957)
    set -l dim   (set_color 7fa392)
    set -l reset (set_color normal)

    set -l stamp /tmp/.ifos-greeted-(id -u)

    if not test -e $stamp; and type -q fastfetch
        fastfetch
        touch $stamp 2>/dev/null
    else
        echo -s $green '  IFOS' $reset $dim '  ·  ' $reset \
                $light 'ifos' $reset $dim ' para os comandos, ' $reset \
                $light 'ifos apps' $reset $dim ' para instalar programas' $reset
    end
end
