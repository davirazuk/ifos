...
command -v maim >/dev/null 2>&1 || {
    command -v notify-send >/dev/null 2>&1 &&
        notify-send -a IFOS "Captura de tela" "O maim não está instalado."
    exit 1
}
...
notify() {  # notify <title> <body> [file]
    ...
    if [[ -n $file ]]; then
        ...
        (
            if [[ $(notify-send --app-name=IFOS --icon="$file" \
                        --hint=string:x-dunst-stack-tag:ifos-screenshot \
                        --action=open=Abrir "$title" "$body") == open ]]; then
                xdg-open "$file" >/dev/null 2>&1
            fi
        ) >/dev/null 2>&1 &
    else
        notify-send --app-name=IFOS \
            --hint=string:x-dunst-stack-tag:ifos-screenshot "$title" "$body"
    fi
}
