...
#
#  Nobody is going to run `ifos-gpu --check` on a machine that looks like it is
...
command -v ifos-gpu >/dev/null 2>&1 || exit 0
...
if notify-send --help 2>&1 | grep -q -- '--action'; then
    chosen=$(notify-send --app-name=IFOS --urgency=critical \
                --hint=string:x-dunst-stack-tag:ifos-gpu \
                --icon=video-display \
                --action=fix="Verificar e consertar" \
                --expire-time=0 \
                "$TITLE" "$BODY")
    [[ $chosen == fix ]] && exec ifos-term "Vídeo e jogos" ifos-gpu --fix
else
